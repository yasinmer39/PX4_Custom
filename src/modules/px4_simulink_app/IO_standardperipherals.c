...
#if IO_STANDARD_MEMORY_OPERATIONS
...
uint8_T standardPeripherals(uint16_T cmdID,uint8_T* payloadBufferRx, uint8_T* payloadBufferTx,uint16_T* peripheralDataSizeResponse, uint8_T* status)
{
    ...
    switch (cmdID) {
        ...
        case WRITEREADSPIMULTIBYTE:
            multiByteWriteReadSPI(peripheralDataSizeResponse, payloadBufferRx, payloadBufferTx, status);
            break;
        ...
        #if IO_STANDARD_MEMORY_OPERATIONS
        case MEMORYREAD:
            memoryRead(payloadBufferRx, payloadBufferTx,peripheralDataSizeResponse);
            break;
        case MEMORYWRITE:
            memoryWrite(payloadBufferRx);
            break;
        case MEMORYBITWRITE:
            memoryBitWrite(payloadBufferRx);
            break;
        case GETBUSWIDTH:
            getBusWidth(payloadBufferTx,peripheralDataSizeResponse,status);
            break;
        #endif
