...
uint8_T addNode(void)
{
    ...
    *  malloc failure will return a NULL value */
    ...
    if(0 == StreamLinkedListVariable.tail)
    {
        /* Adding first Node */
    ...
    if(NULL == pNode->packet.ptrRxPayLoad)
    {
        return 0;
    }
...
uint8_T processRequest(simIOpacket *packet)
{
    ...
    if(isRequestIdCore(packet->requestId))
    {
            ...
            debugPrint("request id is %" PRIu16,packet->requestId);   
        #endif
        switch(packet->requestId)
        {
            case CONNECT:
                /* verify connection is up*/
                connectSimIOResponse((uint16_T*)&packet->dataPayloadSize, &packet->status);
                break;
            ...
            case GETCOREIOSERVERVERSION:
                getCoreVersion((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;

            case GETBOARDIOSERVERVERSION:
                getBoardVersion((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;

            case GETBOARDINFORMATION:
                getBoardName((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;
            ...
            case GETCUSTOMDATAINFORMATION:
                getCustomData((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;
            ...
            case GETHEAPSIZE:
                getHeapsize((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;

            case GETBUFFERCREATIONOVERHEAD:
                getBufferCreationOverhead((uint8_T*)packet->ptrTxPayLoad,(uint16_T*)&packet->dataPayloadSize,&packet->status);
                break;
    ...
    else if(isRequestIdStandardPeripherals((uint16_T)packet->requestId))
    {
        ...
        funNotFound = 1;
            ...
            #if DEBUG_FLAG == 1
                /* Prints a debug message */
                debugPrint("request id is %" PRIu16,_requestId);
            #endif
        ...
        if(readRequest())
        {
            ...
            else if((RcvdPacket.dataPayloadSize != RcvdPacket.peripheralDataSizeResponse && 1 == (RcvdPacket.isRawRead & 0x01))&&(RcvdPacket.ptrTxPayLoad == payloadBufferTx))
            {
                ...
                if(0 == RcvdPacket.status)
                {
                    ...
                    sendPeripheralDatasizeMismatchMessage(&RcvdPacket.requestId,(uint16_T*)&RcvdPacket.dataPayloadSize,(uint16_T*)&RcvdPacket.peripheralDataSizeResponse);
                }
