...
static int socketDataSet(ConnectionData* connection,
                         const void* src,
                         const size_t size,
                         size_t* sizeSent);
...
static int socketDataPending(const SOCKET sock,
                             ConnectionData* connection,
                             int* outPending,
                             int timeoutSecs);

static int serverStreamRecv(ConnectionData* connection, void* dst, size_t size, size_t* sizeRecvd);

static SOCKET serverOpenSocket(int port,
                               char* serverInfoFile,
                               CommsProtocol protocol,
                               int udpSendBufSize,
                               int udpRecvBufSize);
...
#if (!defined(VXWORKS))
static SOCKET clientOpenSocket(char* hostName,
...
static int processArgs(const int argc,
                       void* argv[],
                       char** hostName,
                       unsigned int* portNum,
...
static int initialUDPServerRecvfrom(ConnectionData* connection,
    ...
    *clientSALen = sizeof(*clientSA);
    ...
    nRead = recvfrom(connection->serverData->listenSock, udpPacketBuffer->dataPtr,
                     (size_t)connection->udpData->maxPacketSize, 0U, clientSA, clientSALen);
...
static int socketDataSet(ConnectionData* connection,
    ...
    if (connection->protocol == UDP_PROTOCOL) {
        ...
        if (connection->udpData->isUsingSeqNum) {
             ...
             * sequence number is always transmitted / received in
             * host Endian */
            memcpy(udpPacketBuffer->dataPtr, &connection->udpData->sendSeqNum, UDP_SEQ_NUM_SIZE);
            udpPacketBuffer->dataPtr += UDP_SEQ_NUM_SIZE;
            udpPacketBuffer->dataAvail += UDP_SEQ_NUM_SIZE;
    ...
    if (nSent == SOCK_ERR) {
        ...
        if ((connection->protocol == UDP_PROTOCOL) && (connection->udpData->isUsingSeqNum) &&
            (nSent > 0)) {
...
static SOCKET serverOpenSocket(int port,
    ...
    if (lFd == INVALID_SOCKET) {
         ...
         */
        option = 1;
        sockStatus = setsockopt(lFd, SOL_SOCKET, SO_REUSEADDR, (char*)&option, sizeof(option));
        ...
        if (port == 0) {
            ...
            if (sockStatus == SOCK_ERR) {
                ...
                if (serverInfoFile != NULL) {
                    ...
                    if (fprintfErr < 0) {
                        fprintf(stderr,
                                "Server port number is: %d\nUnable to write server port number to "
                                "output file: %s\n",
                    ...
                    if (fcloseErr == EOF) {
                        fprintf(stderr,
                                "Server port number is: %d\nUnable to close output file after "
                                "writing server port number: %s\n",
                                serverPortNo, strerror(errno));
                        lFd = INVALID_SOCKET;
                    }
...
static void serverAcceptSocket(ConnectionData* connection) {
    ...
    if (connection->serverData->listenSock == INVALID_SOCKET) {
        connection->serverData->listenSock = serverOpenSocket(
            ...
            connection->protocol, connection->udpSendBufSize, connection->udpRecvBufSize);
    }

    /* pass listenSock rather than sock */
    error = socketDataPending(connection->serverData->listenSock, connection, &pending,
                              connection->blockingRecvTimeout);
    ...
    if ((pending > 0) && (error == RTIOSTREAM_NO_ERROR)) {
        if (connection->protocol == TCP_PROTOCOL) {
             ...
             */
            cFd = accept(connection->serverData->listenSock, (struct sockaddr*)&clientAddr,
                         &sFdAddSize);
            ...
            if (error == RTIOSTREAM_ERROR) {
                ...
                if (connect(connection->serverData->listenSock, &clientSA, clientSALen) ==
                    SOCK_ERR) {
                    close(connection->serverData->listenSock);
                    connection->serverData->listenSock = INVALID_SOCKET;
            ...
            if ((strcmp(option, "-hostname") == 0) && (count != argc)) {
                ...
                *isUsingSeqNum = 0;
                ...
                if ((count != 1) || (strncmp(option, "-", 1) == 0)) {
                    printf(
                        "The argument '%s' passed to rtiostream_tcpip is "
...
int rtIOStreamOpen(int argc, void* argv[]) {
    ...
    result = processArgs(argc, argv, &xHostName, &xPortNum, &isClient, &isBlockingRecv,
                         &blockingRecvTimeout, &serverInfoFile, &protocol, &maxPacketSize,
                         &verbosity, &isUsingSeqNum, &udpSendBufSize, &udpRecvBufSize);
    ...
    if (result != RTIOSTREAM_ERROR) { /* polyspace DEFECT:USELESS_IF [Not a defect:Unset] "Needed
                                         for Windows" */
        if (isClient == 1) {
            ...
            sock = serverOpenSocket((int)xPortNum, serverInfoFile, protocol, udpSendBufSize,
                                    udpRecvBufSize);
            if (sock == INVALID_SOCKET) {
                result = RTIOSTREAM_ERROR;
            }
    ...
    if (result != RTIOSTREAM_ERROR) {
        ...
        result = initConnectionData(streamID, isServer, protocol, sock, blockingRecvTimeout,
                                    maxPacketSize, (int)xPortNum, serverInfoFile, verbosity,
